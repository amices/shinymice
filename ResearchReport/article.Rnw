...
Evaluate: bias, variance, and cov rate to find problems that you might miss otherwise. There is no tradeoff between them. Also look at distributional char., and plausibility of imp. values, see \cite{vinknd}.
