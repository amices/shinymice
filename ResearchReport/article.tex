...
R hat not working on MI data? "if over-dispersion does not hold, $\sigma_+^2$ can be too low, which can lead to falsely diagnosing convergence." \cite[p~437]{broo98}. Aim to replicate Lacerda et al. that R hat will not be smaller than 1.1 before iteration nr. 50.
...
bias, variance, cov rate: evaluate problems that you might miss otherwise. there is no tradeoff between them. then your inference is valid. generate data, create miss, usually there is a new method that you want to investigate and then compare to cca. 
...
Simulation code is available in online appendix XYZ on Github.
