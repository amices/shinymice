...
results <- results %>% mutate(crit = qnorm((1 + .95) / 2) / sqrt(t))
...
mean_Rh <- results %>% ggplot(aes(x = t, y = r.hat.max.chain.mean.Y, color = as.factor(p*100))) +
  geom_hline(yintercept = 1,
             color = "grey",
             lwd = 2) +
  geom_hline(yintercept = 1.01, color = "grey", linetype = "dashed", na.rm = TRUE) + 
  # geom_hline(yintercept = 1.1, color = "grey", linetype = "dashed", na.rm = TRUE) + 
  geom_hline(yintercept = 1.2, color = "grey", linetype = "dashed", na.rm = TRUE) + 
  geom_point(size = .25, na.rm = TRUE) +
  geom_line(size = .25, na.rm = TRUE) +
  xlab("") +
...
mean_AC <- results %>% ggplot(aes(x = t, y = ac.max.chain.mean.Y, color = as.factor(p*100))) +
  geom_hline(yintercept = 0,
             color = "grey",
             lwd = 2) +
  geom_line(aes(x = t, y = crit), color = "grey", linetype = "dashed", na.rm = TRUE) + 
  geom_point(size = .25, na.rm = TRUE) +
  geom_line(size = .25, na.rm = TRUE) +
