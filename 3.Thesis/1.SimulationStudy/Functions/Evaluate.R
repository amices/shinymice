...
evaluate.sim <-
  function(sims,
    ...
    if (mean_or_SE == "se") {
      function_to_apply <- function(x) sd(x, na.rm = TRUE) # or use: function(x) sqrt((var(x)))
    ...
    } else {function_to_apply <- function(x) mean(x, na.rm = TRUE)}

    
    # organize output for evaluation
    dt <- map(sims, as.data.table) %>% rbindlist(fill = TRUE) #%>% group_by(t,p)
    # averages
    res <- aggregate(.~t+p, dt, function_to_apply, na.action = na.pass) %>% dplyr::na_if(., "NaN")
