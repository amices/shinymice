...
autocorr_function <- function(imp, maxit, m = 5, n.var = 4, moment = "mean", ac_function = "ac"){
  ...
  for (v in 1:n.var) {
    for (chain in 1:m) {
      if (ac_function == "acf"){
        # use original afc
        ac_per_chain[chain] <-	      
          acf(sims[v, , chain],lag.max = 1, plot = F)$acf[2]
      } else if (ac_function == "ac"){
        # use my own ac
        ac_per_chain[chain] <- ac_lag1(sims[v, , chain])
       }
    }
