...
\begin{abstract}
Multple imputation by chained equations (mice) is a widely used tool to
accommodate missing data. While empirical evidence supports the validity
of inferences obtained using mice, there is no consensus on the
convergence properties of the method. This paper provides insight into
non-convergence of mice algorithms.
\end{abstract}
...
of multiple imputation (MI), proposed by \citet{rubin87}. \textbf{(Call
it mice already, then explain mi and ce sides of mice)} For the
...
filled in) via an \emph{imputation model} (\textbf{term not required to
understand this paper, so circumvent}) . (\textbf{Move this to mi vs.~ce
part in intro:} The imputation model can be a joint model for all
variables, or several univariate conditional distributions (fully
conditional specification). FCS is more flexible and ``avoids the
problem of specifying an appropriate joint imputation distribution and
replaces this by the selection of appropriate univariate conditional
distributions''.)**

\textbf{(Rewrite: With FCS, an iterative algorithmic procedure is used
to \ldots{} multiple imputation by chained equations \ldots{} ``The
values for imputing the missing data are drawn from the posterior
predictive distribution of the missing data conditional on the observed
data.'')}
...
evaluate performance against. \textbf{(Move above: The current practice
of visually inspecting imputation chains for signs of non-mixing or
non-stationarity can only diagnose severely pathological cases of
non-convergence.)}
...
\caption{Convergence diagnostics (means)}\label{fig:unnamed-chunk-2}
\end{figure}
...
\caption{Convergence diagnostics (variances)}\label{fig:unnamed-chunk-3}
\end{figure}
...
\caption{Convergence diagnostics (regression coefficients)}\label{fig:unnamed-chunk-4}
\end{figure}

\hypertarget{convergence-diagnostics-pair-convergnce-diagnostics-with-respective-estimates-not-split-between-convergence-and-simulation-diagnostics}{%
\subsection{Convergence Diagnostics (pair convergnce diagnostics with
respective estimates, not split between convergence and simulation
diagnostics)}\label{convergence-diagnostics-pair-convergnce-diagnostics-with-respective-estimates-not-split-between-convergence-and-simulation-diagnostics}}

\textbf{(Add different percentages missingness??)}
...
divergence (\textbf{move above!}) between subsequent samples in
conditions where \(T<4\). It may not be wise (\textbf{why? and how would
we know? e.g., wider distrubutions? didn't seem like it\ldots{}}) to
terminating the algorithm at that point, but after `recovering' from
this dip in \(AC\)-values. This would imply \(T=6\) as the minimal
...
is the case for \(T=3\), may yield spurious inferences (\textbf{move
above}). Only conditions where \(T>3\) are therefore considered
...
anymore??}). (\textbf{Loess: Similar to the CIWs, there is some trending
across iterations (i.e., the Loess line is never flat)}).
...
(\textbf{add above!!}). \(AC\) would thus not be informative of the
convergence of MI algorithms.

(\textbf{Add above, not here because it was a known limitation: Since
this study only considers only an MCAR missingness mechanism, results
may not be extrapolated to other missing data problems. Proper
