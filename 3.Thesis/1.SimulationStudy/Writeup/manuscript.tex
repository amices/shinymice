...
filled in) several times. Next, the analysis of scientific interest is
performed on each of the completed datasets. And finally, the results of
these analyses are pooled into a single aggregated statistic. The
theoretical foundation of these streps was laid by Rubin
\citeyearpar[1972,][]{rubin87}. For an accessible and comprehensive
introduction to MI from an applied perspective, see e.g.~\citet{buur18}.
In this paper, we focus on the first of these steps: the algorithmic
procedure with which the missing values are imputed.

There is no scientific consensus on the convergence properties of MI
algorithms \citep{taka17}. Some default techniques might not yield
converged states at all \citep{murr18}. Therefore, algorithmic
convergence should be monitored carefully. The current practice is
visually inspecting imputation chains for signs of non-convergence, but
this may be challenging to the untrained eye \citep[\(\S\)
6.5.2]{buur18}. Moreover, visually inspecting imputation chains can only
diagnose severely pathological cases of non-convergence. Therefore,
diagnostic evaluation of convergence would be preferred.

Convergence cannot be determined for sure, because MICE
(\textbf{introduce MICE before!}) is a type of MCMC method, and
...
diagnosed? How many iterations are sufficient/required? What are the
...
For 25\% miss, no bias in estimated means, Rh flat from 30 it. (but
\textless1.1 for it\textgreater11), AC never high but stable after 40
it. Bias in estimated variances only if it=1 or maybe 2?, while Rh of
chain variances is flat around it 25 (smaller than 1.1 for
it\textgreater9), and AC never positive, flat around it 30. Bias in
estimated reg. coeff. only pronounced for the first iteration., while
coverage rates are ok immediately on average, variance explained is
marginally underestimated (bias of 0.5\% on an effect of 19.25\%).
