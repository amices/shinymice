...
knitr::opts_chunk$set(fig.width = 5, echo = FALSE, fig.align = "center") 
...
## Convergence Diagnostics (pair convergnce diagnostics with respective estimates, not split between convergence and simulation diagnostics)

**(Add different percentages missingness??)**
...
CIW is only clearly divergent in the simulation condition $T=1$, see Figure 3B. However, under-estimating the variance of $\bar{Q}$, which is the case for $T=3$, may yield spurious inferences (**move above**). Only conditions where $T>3$ are therefore considered sufficient. (**Possibly irrelevant Loess: These conditions have similar CIWs, but across iterations stability is not established (i.e., the Loess line is never completely flat within the 100 simulation conditions).**)  
...
The observed dip in AC implies that default maxit value of five iterations is the worst possible number of iterations. Moreover, the results of this study imply that assessing the stationarity component of convergence with $AC$ might be redundant, since high $AC$-values are implausible in MI procedures. That is, the randomness induced by the MI algorithm effectively mitigates the risk of dependency within chains (**add above!!**). $AC$ would thus not be informative of the convergence of MI algorithms. 

(**Add above, not here because it was a known limitation: Since this study only considers only an MCAR missingness mechanism, results may not be extrapolated to other missing data problems. Proper performance of the convergence diagnostics under MCAR is necessary but not sufficient to demonstrate appropriateness of $\widehat{R}$ and $AC$ as convergence diagnostics. This is just a proof of concept.**) 
