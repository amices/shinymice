...
will run into a missing data problem [@alli02]. Missingness is problematic because statistical inference cannot be performed on incomplete data without employing \emph{ad hoc} solutions (e.g., list-wise deletion), which may yield wildly invalid results [@buur18]. A popular method to obtain statistically valid results despite of missing data is 'multiple imputation' [MI; @rubin87]. This paper investigates a potential threat to the validity of this method: algorithmic non-convergence.

With multiple imputation, missing data are 'imputed' (i.e., filled in) several times by means of an iterative algorithmic procedure. The method ultimately results in an aggregated statistical inference. For the theoretical foundation of MI, see @rubin87. For an accessible and comprehensive introduction to MI from an applied perspective, see e.g. @buur18.    
...
There is no scientific consensus on the convergence properties of FCS algorithms [@taka17]. Some default techniques in `mice` might not yield converged states at all [@murr18]. Therefore, algorithmic convergence should be monitored carefully. 
...
The stationarity component of convergence may be evaluated with autocorrelation [$AC$; @scha97; @gelm13], numeric standard error [or `MC error'; @gewe92], and Raftery and Lewis's [-@raft91] procedure to determine (**the chain length required to cancel out trending**) the effect of trending within chains.
...
Negative $AC$-values indicate divergence between subsequent samples. It may not be wise (**why? and how would we know? e.g., wider distrubutions? didn't seem like it...**) to terminating the algorithm when AC is negative. High $AC$-values are implausible in MI procedures. That is, the randomness induced by the MI algorithm effectively mitigates the risk of dependency within chains.
...
```{r, include = FALSE}
load("../Results/25percentmiss.Rdata")
source("../2.CreateFigures.R")
```


```{r}
bias_means_plot + R_means_plot + AC_means_plot + plot_layout(nrow = 3) + plot_annotation(title = "Convergence of means, 25% missingness")
```


```{r}
bias_sds_plot + R_sds_plot + AC_sds_plot + plot_layout(nrow = 3) + plot_annotation(title = "Convergence of variances, 25% missingness")
```


```{r}
bias_ests_plot + bias_R_sq_plot + cov_plot + plot_layout(nrow = 3) + plot_annotation(title = "Convergence of effects, 25% missingness")

```

```{r, include = FALSE}
load("../Results/50percentmiss.Rdata")
source("../2.CreateFigures.R")
```


```{r}
bias_means_plot + R_means_plot + AC_means_plot + plot_layout(nrow = 3) + plot_annotation(title = "Convergence of means, 50% missingness")
```


```{r}
bias_sds_plot + R_sds_plot + AC_sds_plot + plot_layout(nrow = 3) + plot_annotation(title = "Convergence of variances, 50% missingness")
```


```{r}
bias_ests_plot + bias_R_sq_plot + cov_plot + plot_layout(nrow = 3) + plot_annotation(title = "Convergence of effects, 50% missingness")

```


```{r, include = FALSE}
load("../Results/75percentmiss.Rdata")
source("../2.CreateFigures.R")
```
...
```{r}
bias_sds_plot + R_sds_plot + AC_sds_plot + plot_layout(nrow = 3) + plot_annotation(title = "Convergence of variances, 75% missingness")
```


```{r}
bias_ests_plot + bias_R_sq_plot + cov_plot + plot_layout(nrow = 3) + plot_annotation(title = "Convergence of effects, 75% missingness")

```


```{r, include = FALSE}
load("../Results/95percentmiss.Rdata")
source("../2.CreateFigures.R")
```


```{r}
bias_means_plot + R_means_plot + AC_means_plot + plot_layout(nrow = 3) + plot_annotation(title = "Convergence of means, 95% missingness")
```


```{r}
bias_sds_plot + R_sds_plot + AC_sds_plot + plot_layout(nrow = 3) + plot_annotation(title = "Convergence of variances, 95% missingness")
```


```{r}
bias_ests_plot + bias_R_sq_plot + cov_plot + plot_layout(nrow = 3) + plot_annotation(title = "Convergence of effects, 95% missingness")
