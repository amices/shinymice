...
Add: MICE is an MCMC method, and convergence of MCMC is not from scalar to a point, but from one distribution to another. Generated values will vary even after convergence.
...
There is no scientific consensus on the convergence properties of multiple imputation algorithms [@taka17]. Some default techniques in `mice` might not yield converged states at all [@murr18]. Therefore, algorithmic convergence should be monitored carefully. The diagnostic evaluation of convergence is preferred over the current practice---visually inspecting imputation chains for signs of non-convergence---since the latter may be challenging to the untrained eye [@buur18, \S~6.5.2]. 

The application of convergence diagnostics to MI methods has not been systematically studied [@buur18]. The measures that are available for diagnosing convergence of iterative algorithmic procedures (Markov chain Monte Carlo methods; e.g., `mice` algorithms or Gibbs samplers) generally evaluate signs of non-convergence [@hoff09]. Non-convergence may be present as non-stationarity within chains (i.e., trending), or as slow mixing between chains (i.e., no intermingling). 
