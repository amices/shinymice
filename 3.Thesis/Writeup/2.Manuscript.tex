...
\noindent We then estimate between-chain variance \(B\) as the variance of the collection of average \(\theta\)s per chain. \textbf{Note that \(B\) refers to the variance in \(\theta\), instead of the typical notation in MI where \(B\) denotes the variance between estimated quantities of scientific interest \(\hat{Q}_{\ell}\).}
