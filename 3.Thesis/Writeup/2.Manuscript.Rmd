...
knitr::include_graphics("../Figures/diagram.pdf") #adapted from [@buur18] par. 1.4.1.
```
...
### $\theta=$ chain mean.
When $\widehat{R}$ and $AC$ are applied to the chain means, **we fail to identify the non-convergence in conditions where $p_{mis}=.95$**. We notice no impact of the missingness proportions. The results only depend on the number of iterations per simulation condition. 
