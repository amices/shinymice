...
plot_bivariate <- function(dat, x, y) {
  ...
  p <- dat %>% ggplot2::ggplot() +
    ...
    scale_y # +
    # ggplot2::labs(tag = "NA") +
    # ggplot2::theme(plot.tag.position = c(0.1, 0.1))
  
  # output
  return(p)
}
plot_bivariate(dat, x = "age", y = "hc") #%>% plotly::ggplotly()#+ ggplot2::annotate(x=27, y=0, label="xyz", color="red") #+annotate(x=27, ymin=-1, ymax=1, color="red")
