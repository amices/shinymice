...
plot_md_pattern <- function(dat) {
  ...
  p <- long_pat %>%
    ...
    scale_y_reverse(
      breaks = 1:max(as.numeric(long_pat$pat_nr)), 
      labels = as.character(n_pat_obs),
      sec.axis = dup_axis(name = "Incomplete variables (per pattern)", labels = as.character(n_vrb_inc)),
      expand = c(0.01,0.01)) +
    ggplot2::scale_x_discrete(limits = vrb, position = "bottom", labels = as.character(n_val_mis)) + #, labels = as.character(n_val_mis)) +
    ggplot2::labs(
      x = "Incomplete cases (per variable)", 
      y = "Pattern frequency"
      ) + #, 
    ...
    theme(plot.margin = margin(25, 10, 10, 10, unit = "pt"))
