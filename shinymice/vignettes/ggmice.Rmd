...
plot_md_pattern <- function(dat) {
  ...
  p <- long_pat %>%
    ...
    ggplot2::scale_x_discrete(limits = vrb, position = "top") + #, labels = as.character(n_val_mis)) +
    ggplot2::labs(x = NULL, y = "frequency", title = "Missing data pattern (patterns as rows, variables as columns)") 
...
imp %>% preprocess_imps(x) %>%
  ...
  ggplot2::facet_wrap(
    ...
    scales = "free_y",
    ncol = 1,
    labeller = ggplot2::labeller(datapoint = c(paste(x, "observed"), paste(x, "missing")) %>% 
...
imp2 <- mice::mice.mids(imp, predictorMatrix = new_pred, maxit = 0)
