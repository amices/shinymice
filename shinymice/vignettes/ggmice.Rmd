...
plot_conditional <- function(dat, x, z) {
  if (is.numeric(dat[[x]])) {
    geom <- ggplot2::geom_histogram(ggplot2::aes(x = .data[[x]], color = conditional, fill = "white"))
  } else {
    geom <-
      ggplot2::geom_bar(
        ggplot2::aes(x = .data[[x]],
                     color = conditional, fill = "white")
      )
  }
  dat[!is.na(dat[[x]]),] %>%
    dplyr::mutate(conditional = factor(
    ...
    geom +
    theme_mice() +
    ggplot2::facet_wrap(~conditional, ncol = 1, scales = "free_y", labeller = ggplot2::labeller(conditional = c(paste(z, "observed"), paste(z, "missing")) %>% setNames( c("Observed", "Missing")))) +
    theme(legend.position = "none")
}

plot_conditional(dat, x = "age", z = "hc")
plot_conditional(dat, x = "reg", z = "hc") # look at skyline plots in mi, and at the legend position --> varname + missing (red) + observed (blue)
```
...
plot_bivariate <- function(dat, x, y) {
  ...
  if (is.numeric(dat[[x]])) {
    ...
    shade_x <- min(dat[[x]], na.rm = TRUE) - .05 * diff(range(dat[[x]], na.rm = TRUE))
    scale_x <- NULL# ggplot2::scale_x_continuous(#expand = c(0.01, 0.01),
  ...
  if (is.numeric(dat[[y]])) {
    ...
    shade_y <- min(dat[[y]], na.rm = TRUE) - .05 * diff(range(dat[[y]], na.rm = TRUE))
    scale_y <- NULL # ggplot2::scale_y_continuous(#expand = c(0.01, 0.01),
  ...
  p <- dat %>% ggplot2::ggplot() +
    ...
    scale_x +
    scale_y +
    ggplot2::annotate("rect", xmin = -Inf, xmax = shade_x, ymin = -Inf, ymax = Inf, alpha = 0.2) +
ggplot2::annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = shade_y, alpha = 0.2)
  # output
  return(p)
}
plot_bivariate(dat, x = "reg", y = "hc")
plot_bivariate(dat, x = "hgt", y = "hc") # instead of .5*SD take 10% of the range instead!
# and add a grey area to distinguish the missing and observed parts of the plot
```
