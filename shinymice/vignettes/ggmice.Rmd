...
# test
p <- plot_bivariate(dat, x = "reg", y = "hc")
...
# preprocess mids object
imps <- imp %>%
  mice::complete("long")

xy_obs <- dat %>%
  cbind(.imp = 0, .id = 1:nrow(.), .) %>%
  .[!is.na(dat[[x]]) &
      !is.na(dat[[y]]),]
xy_imps <- imps[imps$.id %nin% xy_obs$.id, ] 

xy_obs %>%
  ggplot2::ggplot(ggplot2::aes(x = .data[[x]], y = .data[[y]], group = .id)) +
  ggplot2::geom_point(color = mice:::mdc(1)) +
  ggplot2::geom_point(data = xy_imps, color = mice:::mdc(2))  +
  ggplot2::theme_classic()
