...
(vrb <- colnames(pat))
(n_pat_obs <- as.numeric(rownames(pat)))
(n_vrb_inc <- as.numeric(pat[, ncol(pat)]))
(n_cell_mis <- pat[nrow(pat),])
```
...
(pred <- imp$predictorMatrix %>% as.data.frame() %>% cbind(vrb_to_imp = rownames(.), .))
# make the data tidy to plot
(long_pred <- pivot_longer(pred, cols = names(pred)[-1], names_to = "vrb_as_pred"))
# plot the predictor matrix 
long_pred %>% ggplot() + geom_raster(aes(x = vrb_as_pred, y = vrb_to_imp, fill = as.factor(value)))
```
