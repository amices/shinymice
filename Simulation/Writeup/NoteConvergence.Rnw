...
The aim of this note is to provide guidelines for the evaulation of imputations to applied researchers who use MI methods. More specifically, this note adresses the question: `How to diagnose convergence of the MI algorithm implemented in \pkg{mice}?'. 
...
\textbf{Try to put the chain means and vars together in one panel}
...
\textbf{Confidence interval width.} CIW does not show a clear trend across iterations. One might argue that there is some downward trending upto fourty iterations. %After about twenty-five iterations the average CIW across all simulation conditions and repetitions is reached \Sexpr{round(mean(dat[, "CI width"]), 3)}. 
