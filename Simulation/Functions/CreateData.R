...
data.simulation <- function(n = populationsize, true_effect = 2) {
  ...
  # create varcov matrix of the predictors from correlations
  R <- matrix(numeric(3*3), nrow = 3) # will be the correlation matrix
  diag(R) <- 1                        # set diagonal to 1
  xy <- 0.5 # bivar correlations
  xz <- 0.3                            
  yz <- 0.4
  R[upper.tri(R)] <- c(xy, xz, yz)       # fill in x, y, z to upper right
  R[lower.tri(R)] <- c(xy, xz, yz)       # fill in x, y, z to lower left 
  ...
  # compute  outcome var
  simdata$Y <- 1 + true_effect * simdata$X + 0.5 * simdata$Z1 - 1 * simdata$Z2 + rnorm(1)
